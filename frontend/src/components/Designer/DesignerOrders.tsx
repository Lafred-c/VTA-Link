...
import { Upload, Package, Clock, CheckCircle, Hand } from "lucide-react";
...
const DesignerOrders = () => {
  ...
  const { orders, loading, updateStatus, selfAssign } = useOrdersData();
  

  // Backend filters to assigned orders for designer role.
  // But we also want to show the unassigned queue so designers can self-pick.
  // We'll use a second fetch for the queue. For now, mark assigned orders.
  const myOrders = orders.map(o => ({ ...o, assignedDesigner: "Current User" }));

  const stats = {
    assigned: myOrders.length,
    inProgress: myOrders.filter(o => o.status === "Designing").length,
    completed: myOrders.filter(o => !["Designing", "In Queue"].includes(o.status)).length,
  };
  ...
  const handleSelfAssign = async (orderId: string) => {
    ...
    if (r.success) alert("Order assigned to you!");
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      {/* Orders Table with Design Actions */}
