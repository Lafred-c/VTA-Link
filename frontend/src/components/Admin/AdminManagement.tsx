...
import { Search, Eye, Flag, FileText, ChevronDown, X, Check, Trash2, CheckCircle, AlertTriangle, Power } from "lucide-react";
...
function Toast({ message, type, onDone }: { message: string; type: 'success' | 'error'; onDone: () => void }) {
  ...
  return (
    <div className={`fixed bottom-6 right-6 z-[200] flex items-center gap-3 px-5 py-3 rounded-xl shadow-2xl text-white font-semibold text-sm
...
const AdminManagement: React.FC = () => {
  ...
  const {
    ...
    createSupplier, flagSupplier, toggleSupplierActive,
  } = useManagementData();
  ...
  const handleSubmitCreateUser = async () => {
    setFormErr("");
    if (userForm.password !== userForm.confirmPassword) { setFormErr("Passwords do not match"); return; }
    if (!userForm.email || !userForm.password || !userForm.role) { setFormErr("Fill all required fields (Email, Password, Role)"); return; }
    ...
    if (r.success) { showToast("Account created successfully!"); setShowCreateUserModal(false); }
  ...
  const handleUpdateUser = async () => {
    ...
    if (r.success) { showToast("Account updated!"); setShowViewUserModal(false); }
  ...
  const handleDeactivate = async () => {
    ...
    if (r.success) showToast(`${userToDeactivate.firstName} ${userToDeactivate.lastName} deactivated`);
  ...
  const handleSubmitCreateEmp = async () => {
    setFormErr("");
    if (!empForm.fullName || !empForm.position) { setFormErr("Full name and position are required"); return; }
    if (!empForm.role) { setFormErr("Role is required — select Cashier, Designer, Production, or Admin"); return; }
    ...
    if (r.success) { showToast("Employee record created!"); setShowCreateEmpModal(false); }
  ...
  const handleUpdateEmp = async () => {
    ...
    if (r.success) { showToast("Employee updated!"); setShowViewEmpModal(false); }
  ...
  const filteredSuppliers = suppliers.filter(s => {
    ...
    if (a.supplierStatus === 'Active' && b.supplierStatus !== 'Active') return -1;
    if (a.supplierStatus !== 'Active' && b.supplierStatus === 'Active') return 1;
    return 0;
  });
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      <Modal show={!!empToDeactivate} onClose={() => setEmpToDeactivate(null)} title="" width="max-w-md">
        <div className="flex items-center gap-3 mb-4 -mt-4">
          ...
          <div><h3 className="text-xl font-bold">Confirm Deactivation</h3><p className="text-sm text-gray-500">This will disable the employee record</p></div>
        </div>
        {empToDeactivate && (
        ...
        <div className="flex gap-3">
          <button onClick={() => setEmpToDeactivate(null)} className="flex-1 px-4 py-3 bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold rounded-xl">Cancel</button>
          <button onClick={handleDeactivateEmp} className="flex-1 px-4 py-3 bg-red-500 hover:bg-red-600 text-white font-semibold rounded-xl">Deactivate</button>
        </div>
      </Modal>

      {/* ═══ CREATE USER MODAL ═══ */}
      <Modal show={showCreateUserModal} onClose={() => { setShowCreateUserModal(false); setFormErr(""); }} title="Create Account">
        ...
        <div className="flex gap-3 mt-3">
          <button onClick={() => { setShowCreateUserModal(false); setFormErr(""); }} className="flex-1 px-4 py-3 bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold rounded-xl">Cancel</button>
          ...
          <>
            <div className="grid grid-cols-2 gap-4 mb-6">
              ...
              <div>
                ...
                <div className={`px-4 py-2 border rounded-lg text-sm font-semibold ${selectedSupplier.supplierStatus === 'Active' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-red-50 text-red-700 border-red-200'}`}>
            ...
            <div className="flex gap-3">
              <button onClick={() => setShowSupplierInfoModal(false)} className="flex-1 px-4 py-3 bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold rounded-xl">Close</button>
              <button onClick={() => handleToggleSupplierActive(selectedSupplier)}
                ...
                <Power size={16}/>
                {selectedSupplier.supplierStatus === 'Active' ? 'Set Inactive' : 'Reactivate'}
              </button>
            </div>
          </>
        )}
                  ...
                  <tr key={e.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-center">
                      <div className="flex items-center justify-center gap-1">
                        ...
                        {e.isActive && (
                          <button onClick={() => setEmpToDeactivate(e)}
                  ...
                  <tr key={s.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-center">
                      <div className="flex items-center justify-center gap-1">
                        ...
                        <button onClick={() => handleToggleSupplierActive(s)}
                          className={`p-1.5 rounded-lg ${s.supplierStatus === 'Active' ? 'hover:bg-red-100' : 'hover:bg-green-100'}`}
                          title={s.supplierStatus === 'Active' ? 'Set Inactive' : 'Reactivate'}>
                          <Power size={16} className={s.supplierStatus === 'Active' ? 'text-red-500' : 'text-green-500'} />
                        </button>
