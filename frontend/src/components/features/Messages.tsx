...
const Messages: React.FC<MessagesProps> = ({title = "Messages"}) => {
  ...
  const openConversation = useCallback(async (conv: Conversation) => {
    try {
      const messages = await chat.getMessages(conv.userId);
  ...
  useEffect(() => {
    ...
    loadConversations().then((data) => {
      ...
      if (openChatUserId) {
        const target = data.find((c: any) => c.userId === openChatUserId);
        if (target) {
  ...
  useEffect(() => {
    ...
    const subscription = chat.subscribeToMessages(async (payload: any) => {
      ...
      if (
        ...
        const fresh = await chat.getMessages(current.userId);
  ...
  const handleStartDiscovery = async () => {
    ...
    if (potentialRecipients.length === 0 && user) {
      try {
        const list = await chat.getPotentialRecipients(user.role);
  ...
  const handleSendMessage = async () => {
    ...
    if (imageFile) {
      ...
      try {
        uploadedUrl = await chat.uploadChatImage(imageFile);
      } catch (err: any) {
    ...
    try {
      ...
      if (selectedConv.id.startsWith("draft-")) {
        ...
        const real = freshList.find((c: any) => c.userId === selectedConv.userId);
        if (real) {
