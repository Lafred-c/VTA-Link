...
import { Eye, EyeOff, CheckCircle, AlertCircle, User, Mail, Phone, Lock, ArrowRight, Sparkles, Printer, Palette, Zap } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";

/* ── Minimal CSS (non-animation styles only) ────────────────────────── */
const styleTag = document.getElementById('signup-styles') || (() => {
  ...
  return (
    <div className="min-h-screen flex font-['Inter',system-ui,sans-serif]">
      ...
      {/* ══════════════════════════════════════════════════════════════
         ...
         ══════════════════════════════════════════════════════════════ */}
      <div className="hidden lg:flex flex-1 relative overflow-hidden flex-col justify-between p-10 xl:p-12"
        style={{ background: 'linear-gradient(160deg, #AA00FD 0%, #E80088 40%, #C70070 70%, #AA00FD 100%)' }}
      >
        {/* Decorative floating shapes */}
        <motion.div {...floatVariant(20, 8, 6)} animate="animate"
          className="absolute" style={{ top: '8%', right: '12%', width: 80, height: 80, borderRadius: '24px', background: 'rgba(255,209,2,.25)' }} />
        <motion.div {...floatVariant(28, -6, 8)} animate="animate"
          className="absolute" style={{ top: '35%', right: '8%', width: 60, height: 60, borderRadius: '50%', background: 'rgba(0,190,244,.2)' }} />
        <motion.div {...floatVariant(16, 12, 7)} animate="animate"
          className="absolute" style={{ bottom: '20%', left: '8%', width: 100, height: 100, borderRadius: '28px', background: 'rgba(255,255,255,.08)', border: '1px solid rgba(255,255,255,.15)' }} />
        <motion.div {...floatVariant(28, -6, 8)} animate="animate"
          className="absolute" style={{ top: '60%', left: '30%', width: 40, height: 40, borderRadius: '50%', background: 'rgba(255,209,2,.15)' }} />
        <motion.div {...pulseRing(0)} animate="animate"
          className="absolute" style={{ top: '15%', left: '15%', width: 180, height: 180, borderRadius: '50%', border: '1px solid rgba(255,255,255,.1)' }} />
        <motion.div {...pulseRing(2)} animate="animate"
          className="absolute" style={{ bottom: '10%', right: '10%', width: 140, height: 140, borderRadius: '50%', border: '1px solid rgba(255,255,255,.08)' }} />
          ...
          <div className="space-y-2">
            {features.map(({ icon: Icon, label, desc }, i) => (
              <motion.div key={label}
                variants={fadeUp} initial="hidden" animate="visible" custom={0.2 + i * 0.08}
                whileHover={{ x: 4 }}
        ...
        <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={0.4}
          ...
          <span className="su-link cursor-pointer transition-colors">Terms</span>
          <span className="su-link cursor-pointer transition-colors">Privacy</span>
          <span className="su-link cursor-pointer transition-colors">Contact</span>
        </motion.div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center px-5 py-6 sm:px-8 bg-gray-50/80 min-h-screen overflow-y-auto">
        <div className="w-full max-w-[420px]">
          ...
          <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={0}
            ...
            <div className="inline-flex items-center gap-2">
              <div className="w-9 h-9 rounded-xl flex items-center justify-center"
                style={{ background: 'linear-gradient(135deg, #E80088, #AA00FD)' }}>
              ...
              {successMessage && (
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-xs text-emerald-600 mt-1 leading-relaxed">{successMessage}</p>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
            ...
            {!successMessage && (
              <form onSubmit={handleSubmit} className="space-y-3.5">
                ...
                <motion.p variants={fadeUp} initial="hidden" animate="visible" custom={0.45}
                  ...
                  <span className="text-gray-500 font-medium cursor-pointer hover:text-[#E80088] transition-colors">Terms of Service</span>
                  {' '}and{' '}
                  <span className="text-gray-500 font-medium cursor-pointer hover:text-[#E80088] transition-colors">Privacy Policy</span>.
                </motion.p>

                {/* ── Divider ─────────────────────────────────── */}
                <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={0.48}
          ...
          {/* Bottom brand mark */}
          <motion.p variants={fadeUp} initial="hidden" animate="visible" custom={0.55}
